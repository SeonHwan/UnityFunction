...
namespace NakiedLib.FirebaseAddressables
{
    public static class FirebaseAddressablesCache
    {
        ...
        public static string IdTransformFunc(IResourceLocation location)
        {
            ...
            Debug.Log($"To Be Tranfered Internal ID : {location.InternalId}");
        ...
        {
            PreWarmDependencies(new List<object>() { key }, fbStorage, completed);
        }

        public static void PreWarmDependencies(List<object> keys, FirebaseStorage fbStorage, Action completed)
        {
            if (FirebaseAddressablesManager.IsFirebaseSetupFinished == false)
            {
                FirebaseAddressablesManager.FirebaseSetupFinished += () => { PreWarmDependencies(keys, fbStorage, completed); };
            }
            else
            {
                ...
                {
                    initOperation.Completed += handle => GetFirebaseUrl(keys, fbStorage, completed);
                }
                else
                {
                    GetFirebaseUrl(keys, fbStorage, completed);
                }
            }
        }
